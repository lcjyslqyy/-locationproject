...
{
    /// <summary>
    /// 数据访问抽象基础类(基于Peta的conectstring)
    /// </summary>
    public abstract class PetaSQLHelper
    {
        public PetaSQLHelper()
        {
        }
        #region 公用方法
        /// <summary>
        ...
        public static bool Exists(Base_Helper.PetaPoco.Database PCDb,string strSql, params SqlParameter[] cmdParms)
        {
            object obj = PetaSQLHelper.GetSingle(PCDb,strSql, cmdParms);
            int cmdresult;
            if ((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
            {
                cmdresult = 0;
        ...
        public static int ExecuteSqlByTime(Base_Helper.PetaPoco.Database PCDb,string SQLString, int Times)
        {
            using (SqlConnection connection = new SqlConnection(PCDb._helperconnectionString))
            {
                using (SqlCommand cmd = new SqlCommand(SQLString, connection))
        ...
        public static object GetSingle(Base_Helper.PetaPoco.Database PCDb, string SQLString)
        {
            using (SqlConnection connection = new SqlConnection(PCDb._helperconnectionString))
            {
                using (SqlCommand cmd = new SqlCommand(SQLString, connection))
        ...
        public static DataSet Query(Base_Helper.PetaPoco.Database PCDb,string SQLString)
        {
            using (SqlConnection connection = new SqlConnection(PCDb._helperconnectionString))
            {
        ...
        public static DataSet Query(Base_Helper.PetaPoco.Database PCDb,string SQLString, int Times)
        {
            using (SqlConnection connection = new SqlConnection(PCDb._helperconnectionString))
            {
                DataSet ds = new DataSet();
        ...
        /// </summary>
        /// <param name="SQLStringList">SQL语句的哈希表（key为sql语句，value是该语句的SqlParameter[]）</param>
        public static void ExecuteSqlTranWithIndentity(Base_Helper.PetaPoco.Database PCDb,System.Collections.Generic.List<CommandInfo> SQLStringList)
        {
            using (SqlConnection conn = new SqlConnection(PCDb._helperconnectionString))
            {
                conn.Open();
        ...
        public static DataSet RunProcedure(Base_Helper.PetaPoco.Database PCDb,string storedProcName, IDataParameter[] parameters, string tableName, int Times)
        {
            using (SqlConnection connection = new SqlConnection(PCDb._helperconnectionString))
            {
                DataSet dataSet = new DataSet();
        ...
        /// <param name="parameters">存储过程中的参数数组</param>
        /// <returns>输出参数</returns>
        public static string ExecuteProcedure(Base_Helper.PetaPoco.Database PCDb,string storedProcName, SqlParameter[] parameters, int Times)
        {
            using (SqlConnection connection = new SqlConnection(PCDb._helperconnectionString))
            {
                connection.Open();
        ...
        /// <param name="row">一行</param>
        /// <returns>当前实体</returns>
        public static T ConversionModel<T>(T Obj, DataRow row, DataTable dt)
        {
            ...
            foreach (PropertyInfo propertyInfo in properites)//遍历数组
            {
                if (dt.Columns[propertyInfo.Name] != null)
                {
                    if (propertyInfo.PropertyType == typeof(int))
                        propertyInfo.SetValue(Obj, int.Parse(row[propertyInfo.Name].ToString()), null);
                    if (propertyInfo.PropertyType == typeof(decimal))
                        propertyInfo.SetValue(Obj, decimal.Parse(row[propertyInfo.Name].ToString()), null);
                    if (propertyInfo.PropertyType == typeof(string))
                        propertyInfo.SetValue(Obj, row[propertyInfo.Name].ToString(), null);
                    if (propertyInfo.PropertyType == typeof(double))
                        propertyInfo.SetValue(Obj, double.Parse(row[propertyInfo.Name].ToString()), null);
                    if (propertyInfo.PropertyType == typeof(float))
                        propertyInfo.SetValue(Obj, float.Parse(row[propertyInfo.Name].ToString()), null);
                    if (propertyInfo.PropertyType == typeof(bool))
                        propertyInfo.SetValue(Obj, bool.Parse(row[propertyInfo.Name].ToString()), null);
                    if (propertyInfo.PropertyType == typeof(DateTime))
                    {
                        try
                        {
                            propertyInfo.SetValue(Obj, DateTime.Parse(row[propertyInfo.Name].ToString()), null);
                        }
                        catch { propertyInfo.SetValue(Obj, DateTime.Now, null); }
                    }
                }
            }
            return Obj;
